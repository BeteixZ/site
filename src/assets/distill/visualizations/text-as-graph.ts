...
export class TextAsGraph {
  ...
  private adjMatSel;
  private fontSize = 30
  private charWidth = this.calcCharWidth(this.fontSize);
  ...
  constructor() {
    ...
    const inputSel = divSel.append('input')
      .st({ 'word-spacing': wordSpacing + 'px', fontSize: this.fontSize })
      .at({ maxlength: 30 })
      ...
      .on('mousemove', function () {
        ...
        for (let l of [...this.value]) {
          const isSpace = l == ' ';
          offset += isSpace ? this.charWidth + wordSpacing : this.charWidth;
          wordIdx += isSpace ? 1 : 0;
          if (offset > x) {
...
private calcCharWidth(fontSize: number) {
  if (typeof document === 'undefined') return 8;  // Node / SSR 兜底
  const spanSel = d3.select('body').append('span').text('x')
    .style('font-family', 'monospace')
    .style('font-size', `${fontSize}px`);
  const node = spanSel.node();
  const w = node ? node.offsetWidth : 8;   // 保底
  spanSel.remove();
  return w;
}
