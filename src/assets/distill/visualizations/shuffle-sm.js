...
export function shuffleSm(){
  ...
  function drawGraph(){
    ...
    function setPermutation(permuteArray){
      ...
      nodes.forEach(d => {
  // ✅ 确保 d.posI 始终是一个函数，避免 undefined
  d.posI = d.posI || (i => d.pos); // 如果没定义，就保持原位置不动
});
...
try {
      ...
      // ✅ 更新 posI 为真正的路径插值函数
      d.posI = i => {
        const pt = node.getPointAtLength(i * pathLength);
        return [pt.x, pt.y];
      };
    } catch (e) {
      console.warn("Failed to compute path length", e);
    }
    });
  });
